...
| `evaluation_jobs` | Job nền để AI evaluator đánh giá một phiên bản câu hỏi |
  ...
  generation_presets: [
    {
      ...
      name: "Ôn tập cây nhị phân",
      planItems: [
        { questionTypeId: "mcq", bloomId: "understand", count: 5 }
      ],
...
- `generation_presets` là danh sách preset sinh câu hỏi theo tài khoản, giới hạn 12 mẫu/user; localStorage frontend chỉ là cache/fallback.
...
`questions` không chứa nội dung câu hỏi. Collection này chỉ giữ identity, con trỏ version và summary phục vụ truy vấn nhanh. Backend hiện tại lưu `created_by_user_id` ở aggregate để lọc ownership P0, nhưng chưa denormalize `current_classification`; danh sách câu hỏi vẫn lấy aggregate `questions` rồi `$lookup` sang `question_versions` theo `current_version_id`.

Câu hỏi do AI sinh mặc định ở `DRAFT` để Teacher rà soát; chỉ khi Teacher/Admin gọi `submit-review` thì trạng thái chuyển sang `PENDING`.
...
```javascript
{
  ...
  dedupe_key: "sha256(version_id|content_hash|model)",
  status: "QUEUED", // QUEUED | PROCESSING | COMPLETED | ERROR | STALE | CANCELLED
  trigger: "TEACHER_SUBMIT", // hoặc REVIEWER_REQUEST
  requested_by_user_id: ObjectId("..."),
  evaluator_model_code: "deepseek-r1", // mac dinh tu EVALUATION_MODEL_PROVIDER, co the la "ollama:<model>"
  policy_snapshot: {
    id: ObjectId("..."),
    name: "Default question quality policy",
    version: 1,
    weights: {
      faithfulness: 0.35,
      contextual_relevancy: 0.20,
  ...
  prompt_snapshot: {
    template_key: "evaluation:question_quality",
    template_hash: "sha256...",
    rendered_prompt_hash: "sha256...",
    rendered_prompt_chars: 4200,
    source_limit: 3,
  ...
  attempt_no: 1,
  max_attempts: 1,
  result: { evaluation_id: ObjectId("..."), passed: true, overall_score: 0.86, color: "GREEN" },
  error: { message: "timeout/parser error...", at: ISODate() },
  queued_at: ISODate(),
  started_at: ISODate(),
  finished_at: ISODate(),
  duration_ms: 31120,
...
`evaluation_jobs` là hàng đợi/outbox cho AI evaluator. Khi Teacher gửi duyệt hoặc Reviewer bấm chạy AI, backend tạo job cho `current_version_id`; worker chỉ ghi `question_evaluations` nếu version đó vẫn là current version.

### 5.18. `question_evaluations`
...
`prompt_snapshot` lưu hash/kích thước thay vì nhét toàn bộ prompt vào DB; nếu production cần audit đầy đủ thì chuyển prompt render sang artifact storage có hash. `raw_model_response` cũng phải có giới hạn byte hoặc cờ `truncated` nếu model trả quá dài.

### 5.19. `question_reviews`
...
Upload dùng trình tự: ghi file tạm → kiểm tra loại/kích thước/hash → chuyển atomic sang artifact storage → insert/update metadata MongoDB → enqueue OCR. Backend hiện dùng `POST /api/v1/documents/upload` làm endpoint nghiệp vụ chính cho giảng viên đăng tài liệu; `POST /api/v1/ocr/upload` chỉ giữ làm compatibility route cho client/script cũ. Nếu ghi metadata thất bại, file chưa được tham chiếu phải được garbage collector có grace period thu hồi; không xóa artifact đã được MongoDB tham chiếu.
...
stateDiagram-v2
    state "Review" as Review {
        [*] --> DRAFT
        DRAFT --> PENDING: Teacher submit-review
        PENDING --> APPROVED
        PENDING --> REJECTED
        PENDING --> NEEDS_REVISION
        NEEDS_REVISION --> PENDING: Teacher submit-review lại
...
ragDb.questions.createIndex(
  { created_by_user_id: 1, updated_at: -1 },
  { name: "ix_questions_owner" }
);
ragDb.questions.createIndex(
...
ragDb.evaluation_jobs.createIndex(
  { status: 1, queued_at: 1 },
  { name: "ix_evaluation_jobs_queue" }
);
ragDb.evaluation_jobs.createIndex(
  { dedupe_key: 1 },
  {
    ...
    partialFilterExpression: { $or: [{ status: "QUEUED" }, { status: "PROCESSING" }] }
  }
);
...
5. Giữ `review_status = DRAFT` hoặc `NEEDS_REVISION` nếu câu hỏi đang ở trạng thái Teacher còn sửa; nếu chỉnh phiên bản đã có quyết định thì đặt lại `review_status = PENDING` để review lại.
...
| `backend/prompts/*.txt` | system, Bloom, question type, example, output format | `prompt_templates` | Bootstrap P1 seed nội dung file version 1; runtime mặc định đọc file local qua `PROMPT_SOURCE=file`, chỉ đọc DB active trước khi cấu hình `PROMPT_SOURCE=db` |
| LLM factory | `qwen`, `deepseek`, `deepseek-r1`, `ollama:<model>`, `gemini` do client/config chọn | `ai_models` + routing capability | Bootstrap P1 seed `qwen`/`deepseek`/`deepseek-r1`; evaluator mặc định lấy từ `EVALUATION_MODEL_PROVIDER`, production chỉ bật local, Gemini là provider thử nghiệm |

Hiện backend đã có bootstrap V2, `users`, `documents`, `document_jobs`, `document_pages`, `chunk_sets`, `document_chunks`, `vector_collections`, `chunk_embeddings`, `generation_jobs`, `generation_runs`, `questions`, `question_versions`, `evaluation_jobs`, `question_evaluations`, `question_reviews`, catalog APIs và audit khi review/evaluation. Các phần đã thực hiện, còn thiếu hoặc chưa nên claim hoàn tất được tách riêng trong file `DATABASE_DESIGN_V2_GAP_PRIORITY.md`.
...
2. `ai_models`, `prompt_templates` và `evaluation_policies` đã có collection, seed, cấu hình `PROMPT_SOURCE=file|db` và UI quản trị demo; còn thiếu workflow promote/rollback version có kiểm duyệt.
3. AI evaluation đã có endpoint nhận scores/feedback và endpoint enqueue tự chấm bằng local LLM cấu hình qua `EVALUATION_MODEL_PROVIDER` (mặc định `deepseek-r1`, hỗ trợ `ollama:<model>`); nếu model local lỗi thì lưu `ERROR` để retry/override thủ công, production vẫn cần benchmark/hardening evaluator.
...
5. `moodle_publications` đã có API P0 ghi nhận mock publication kèm export payload GIFT/XML; chưa có worker/API gọi Moodle thật.
6. Endpoint upload nghiệp vụ chính là `POST /api/v1/documents/upload` và hiện chỉ nhận PDF; DOC/DOCX phải thêm converter/sanitizer hoặc công bố PDF-only cho release đầu. API đã nhận `subject_id`/`chapter_id` dạng form field, nhưng Generate page chưa có UI chọn metadata này lúc upload.
...
8. Generate page đã hiển thị thời gian xử lý trong phiên UI hiện tại và backend persist timing client/server bước đầu trong `generation_jobs.metrics`; chưa có dashboard/aggregation telemetry nhiều lần chạy.
9. Frontend đã có route guard và login redirect theo route vừa bấm; trước cutover vẫn cần E2E UI/auth thật cho guest -> login -> quay lại route đúng role.
10. Ownership tài liệu/câu hỏi đã siết cho Teacher ở API P0; production còn cần backfill owner cho dữ liệu legacy và chính sách chia sẻ/chuyển owner nếu nhiều giảng viên dùng chung học liệu.
...
4. Import prompt theo `template_key + version + content_hash`, gồm `system`, `output_format`, `bloom/*`, `question_type/*` và `evaluation/*`; không ghi đè prompt đã dùng.
...
Seed dữ liệu phục vụ demo tách khỏi bootstrap production. Hiện có `backend/scripts/database/seed_demo_users.py` để tạo tài khoản `admin/admin`, `reviewer/reviewer`, và `backend/scripts/database/seed_demo_review_flow.py` để refresh câu hỏi `Q-DEMO-REVIEW-001` ở trạng thái `PENDING`/`NOT_STARTED` cho luồng Reviewer chạy AI evaluation.
...
- API dành cho Teacher phải lọc list theo owner và kiểm tra ownership trước các thao tác dùng tài liệu/câu hỏi; Reviewer/Admin giữ quyền đọc/kiểm duyệt theo role.
...
- E2E UI/auth: guest chỉ thấy menu công khai, truy cập route cần quyền thì chuyển login; login Google thành công thì quay lại route nếu role có quyền hoặc fallback theo role (`Teacher` -> `/sinh-cau-hoi`, `Reviewer` -> `/kiem-duyet`, `Admin` -> `/danh-muc`) và header mở nhóm menu đúng role.
...
- Teacher không xem/sửa/tạo câu hỏi từ tài liệu thuộc Teacher khác, trừ khi sau này có policy chia sẻ/chuyển owner được thiết kế rõ.
