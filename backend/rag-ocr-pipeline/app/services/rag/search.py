...
from app.engine.chromadb_engine import _get_chroma_client as get_chroma_client
...
def get_context_for_generation(document_id: str, collection_name: str, target_heading: str = None, min_density: float = 0.0, limit: int = 5) -> str:
    ...
    # OLD: collection = client.get_or_create_collection(name="chunk")
    #resolved_collection = settings.chromadb_collection_name or "chunks"
    ...
    try:
        ...
        # OLD: filtered_chunks.sort(key=lambda x: x[1].get("page_start", 0) if x[1].get("page_start") else 0)
        ...
        # OLD:
        # for doc, meta in chunks_with_meta:
        #     heading = f"[{' > '.join(meta.get('heading_path', []))}]" if meta.get('heading_path') else ""
