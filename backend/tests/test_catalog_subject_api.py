...
def current_user(role: str, permissions: tuple[str, ...] | None = None) -> CurrentUser:
    user_id = ObjectId()
    user_permissions = permissions
    if user_permissions is None:
        user_permissions = ("admin.catalog",) if role == "Admin" else ("catalog.subjects.manage_own",)
    return CurrentUser(
        id=user_id,
...
def test_teacher_creates_subject_and_payload_is_normalized():
...
def test_subject_manager_permission_can_list_subjects():
    service = FakeCatalogService()
    user = current_user("Custom", ("catalog.subjects.manage_own",))
    client = make_client(user, service)

    response = client.get("/api/v1/catalog/subjects")

    assert response.status_code == 200
    assert response.json()[0]["subject_code"] == "CTDL"
